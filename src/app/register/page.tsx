  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (error) {
      if (error.message.toLowerCase().includes("already registered") || error.status === 422) {
        alert("Pendaftaran Gagal: Email ini sudah terdaftar di sistem Syncra AI! Silakan gunakan email lain atau langsung login.");
      } else {
    ...
    if (data?.user) {
      ...
      alert("Pemberitahuan: Silakan periksa kotak masuk Gmail Anda untuk melihat tautan aktivasi akun.");
      setIsSent(true);
    }
    ...
    return (
      <div className="min-h-screen bg-[#050505] flex items-center justify-center p-4">
        <div className="glass-card max-w-md p-10 text-center space-y-4">
          ...
          <h2 className="text-2xl font-bold text-white uppercase italic">Check Your Email</h2>
          <p className="text-gray-400 text-sm">Kami telah mengirimkan link verifikasi ke <span className="text-white font-bold">{formData.email}</span>. Silakan klik link tersebut untuk mengaktifkan akun Syncra kamu.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="glass-card w-full max-w-md p-10 border-cyan-500/20 relative overflow-hidden">
        <div className="relative z-10 space-y-8">
          <div className="text-center">
            <h1 className="text-3xl font-black text-white italic uppercase">Join Syncra <span className="text-cyan-500 text-glow">AI</span></h1>
            <p className="text-gray-500 text-[9px] font-bold uppercase tracking-[0.3em] mt-2">Create Administrative Account</p>
          </div>

          <form onSubmit={handleRegister} className="space-y-4">
            {/* INPUT NAMA */}
            <div className="space-y-1">
              <label className="text-[10px] text-gray-500 font-black uppercase ml-1">Full Name</label>
            ...
            {/* INPUT EMAIL */}
            <div className="space-y-1">
              <label className="text-[10px] text-gray-500 font-black uppercase ml-1">Email</label>
              <div className="relative">
            ...
            {/* INPUT PASSWORD */}
            <div className="space-y-1">
              <label className="text-[10px] text-gray-500 font-black uppercase ml-1">Password</label>
              <div className="relative">
